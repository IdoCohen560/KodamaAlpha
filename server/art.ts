/**
 * Kodama ASCII art for all 25 species
 *
 * Each species has 3 animation frames (idle variations) for Hatchling+ stage.
 * Each frame is 5 lines, ~12 chars wide.
 * {E} is replaced with the eye character at render time.
 *
 * Egg art (Evolution Stage 1, levels 1-4): 2-line species-tinted eggs.
 */

import type { Species, Eye, Hat, Rarity, StatName, BuddyBones } from "./engine.ts";

// ─── Egg art (Stage 1, used when level < 5) ────────────────────────────────

export const EGG_ART: Record<Species, string[][]> = {
  duck:      [["   ___   ", "  / ~ \\  ", " ( {E}.{E} ) ", "  \\___/  ", "         "]],
  goose:     [["   ___   ", "  / ^ \\  ", " ( {E}.{E} ) ", "  \\___/  ", "         "]],
  blob:      [["   ___   ", "  / . \\  ", " ( {E}.{E} ) ", "  \\___/  ", "         "]],
  cat:       [["   ___   ", "  /\\ /\\  ", " ( {E}.{E} ) ", "  \\___/  ", "         "]],
  ...
  ghost:     [["   ___   ", "  / . \\  ", " ( {E}.{E} ) ", "  \\___/  ", "  ~`~`   "]],
  axolotl:   [["   ___   ", " }/ . \\{ ", " ({E}.{E}) ", "  \\___/  ", "         "]],
  capybara:  [["   ___   ", "  / o \\  ", " ( {E}.{E} ) ", "  \\___/  ", "         "]],
  cactus:    [["   ___   ", "  /| |\\  ", " ( {E}.{E} ) ", "  \\___/  ", "         "]],
  robot:     [["   ___   ", "  /[=]\\  ", " [ {E}.{E} ] ", "  \\___/  ", "         "]],
  rabbit:    [["   ___   ", "  /| |\\  ", " ( {E}.{E} ) ", "  \\___/  ", "         "]],
  mushroom:  [["   ___   ", "  /oOo\\  ", " ( {E}.{E} ) ", "  \\___/  ", "         "]],
  chonk:     [["   ___   ", "  / . \\  ", " ({E} .{E} )", "  \\___/  ", "         "]],
  fox:       [["   ___   ", "  /\\ /\\  ", " ({E}w{E}) ", "  \\___/  ", "    ~    "]],
  bat:       [["   ___   ", " \\/ . \\/  ", " ({E}v{E}) ", "  \\___/  ", "         "]],
  panda:     [["   ___   ", "  /o o\\  ", " ({E}.{E}) ", "  \\___/  ", "         "]],
  phoenix:   [["   ___   ", "  /~^~\\  ", " ({E}^{E}) ", "  \\___/  ", "   ~~    "]],
  wolf:      [["   ___   ", "  /\\ /\\  ", " ({E}w{E}) ", "  \\___/  ", "         "]],
  slime:     [["   ___   ", "  / . \\  ", " .({E}{E}). ", "  \\___/  ", "  ~~~~   "]],
  crystal:   [["   /\\    ", "  / .\\   ", " /{E}|{E}\\ ", "  \\__/   ", "         "]],
...
// Shiny colors — distinct from all rarity colors (gray/green/blue/purple/gold)
const SHINY_COLORS_ANSI: Record<string, string> = {
  cyan:   "\x1b[38;2;0;229;255m",    // #00e5ff — electric cyan
  red:    "\x1b[38;2;255;69;58m",     // #ff453a — vivid red
  yellow: "\x1b[38;2;255;214;10m",    // #ffd60a — bright yellow
};
const SHINY_COLOR_DEFAULT = SHINY_COLORS_ANSI.cyan;

function getShinyColor(shinyColor?: string): string {
  return SHINY_COLORS_ANSI[shinyColor ?? "cyan"] ?? SHINY_COLOR_DEFAULT;
}
const BOLD = "\x1b[1m";
const DIM = "\x1b[2m";
const NC = "\x1b[0m";
...
export function renderCompanionCard(
  ...
  const shiny = bones.shiny ? `${getShinyColor(bones.shinyColor)}\u2728 ${NC}` : "";
...
export function renderCompanionCardMarkdown(
  ...
  const stars = RARITY_STARS[bones.rarity];
  const shinyEmoji = bones.shinyColor === "red" ? "\uD83D\uDD34" : bones.shinyColor === "yellow" ? "\uD83D\uDFE1" : "\u2728";
  const shiny = bones.shiny ? ` ${shinyEmoji}` : "";
  const art = getArtFrame(bones.species, bones.eye, frame);
