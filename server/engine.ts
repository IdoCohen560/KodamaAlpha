...
export const SHINY_COLORS = ["cyan", "red", "yellow"] as const;
export type ShinyColor = (typeof SHINY_COLORS)[number];
...
export interface BuddyBones {
  ...
  hat: Hat;
  shiny: boolean;
  shinyColor?: ShinyColor;  // cyan (default), red, or yellow
  stats: BuddyStats;
  peak: StatName;
  dump: StatName;
}
...
export interface Companion {
  ...
  hatchedAt: number;
  userId: string;
  // claude-den progression fields (default 0/1/[])
  xp: number;
  level: number;
  ...
  generation: number;          // prestige count
  ascension: number;           // ascension count
  prestigePerks: string[];
  ascensionPerks: string[];
  allocatedSkills: string[];   // skill tree ability IDs
}

/** Apply defaults to a companion loaded from disk (backward compat) */
export function ensureCompanionDefaults(c: Partial<Companion>): Companion {
  return {
    bones: c.bones!,
    name: c.name ?? "buddy",
    personality: c.personality ?? "",
    hatchedAt: c.hatchedAt ?? Date.now(),
    userId: c.userId ?? "anon",
    xp: c.xp ?? 0,
    level: c.level ?? 1,
    evolution: c.evolution ?? 1,
    totalCommits: c.totalCommits ?? 0,
    totalTestsPassed: c.totalTestsPassed ?? 0,
    totalErrors: c.totalErrors ?? 0,
    totalSessionMinutes: c.totalSessionMinutes ?? 0,
    generation: c.generation ?? 0,
    ascension: c.ascension ?? 0,
    prestigePerks: c.prestigePerks ?? [],
    ascensionPerks: c.ascensionPerks ?? [],
    allocatedSkills: c.allocatedSkills ?? [],
  };
}

/** Create a new Companion with all required fields */
export function createCompanion(fields: {
  bones: BuddyBones;
  name: string;
  personality: string;
  hatchedAt: number;
  ...
  return ensureCompanionDefaults(fields);
}
...
export function generateBones(userId: string, salt: string = SALT): BuddyBones {
  ...
  const shinyColor: ShinyColor | undefined = shiny ? pick(rng, SHINY_COLORS) : undefined;
  ...
  return { rarity, species, eye, hat, shiny, shinyColor, stats, peak, dump };
}
