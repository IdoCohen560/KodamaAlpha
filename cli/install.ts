...
import { generateBones, renderBuddy, renderFace, RARITY_STARS, createCompanion } from "../server/engine.ts";
...
function installStatusLine(settings: Record<string, any>) {
  const compositeScript = join(PROJECT_ROOT, "statusline", "composite-status.sh");

  settings.statusLine = {
    type: "command",
    command: compositeScript,
    padding: 1,
    refreshInterval: 1,  // 1 second — drives the buddy animation
  };

  // Auto-detect ruflo statusline and register as addon
  const addonsFile = join(homedir(), ".claude-buddy", "statusline-addons.json");
  const rufloStatusline = join(homedir(), ".claude", "statusline.sh");
  const addons: Array<{ name: string; command: string; enabled: boolean }> = [];

  if (existsSync(rufloStatusline)) {
    addons.push({ name: "ruflo", command: rufloStatusline, enabled: true });
    ok("Detected ruflo statusline — registered as addon");
  }

  mkdirSync(join(homedir(), ".claude-buddy"), { recursive: true });
  writeFileSync(addonsFile, JSON.stringify({ addons }, null, 2));

  ok("Composite status line configured (buddy + addons)");
}
...
function installHooks(settings: Record<string, any>) {
  ...
  const sessionHook  = join(PROJECT_ROOT, "hooks", "session-track.sh");
  ...
  settings.hooks.UserPromptSubmit.push({
    hooks: [{ type: "command", command: sessionHook }],
  });

  ok("Hooks registered: PostToolUse + Stop + UserPromptSubmit + SessionTrack");
}
